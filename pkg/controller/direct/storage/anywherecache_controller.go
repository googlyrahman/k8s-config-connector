...
func (a *AnywhereCacheAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	log.V(2).Info(fmt.Sprintf("Updating AnywhereCache - %s", a.id.String()))
	mapCtx := &direct.MapContext{}
	...
	*/
	...
	if currentState == anywhereCacheStateCreating {
		log.V(2).Info("Delaying update as AnywhereCache is in 'creating' state.")
	...
	*/
	if a.IsStateChangeRequested() {
		...
		if len(paths) != 0 || a.actual.GetPendingUpdate() {
			...
			log.V(2).Info("State change completed, but metadata updates pending or in progress. Requeuing.")
		...
		log.V(2).Info("State change completed, no further updates pending.")
	...
	if len(paths) == 0 {
		...
		log.V(2).Info("No metadata fields need update.")
		return updateOp.UpdateStatus(ctx, status, getCondition(v1.ConditionTrue, k8s.UpToDate, k8s.UpToDateMessage))
		...
		log.V(2).Info("Delaying metadata update, as a previous update is already in progress.")
		return updateOp.UpdateStatus(ctx, status, getCondition(v1.ConditionTrue, k8s.Updating, k8s.UpdatingMessage))
...
func (a *AnywhereCacheAdapter) Export(ctx context.Context) (*unstructured.Unstructured, error) {
	...
	return nil, fmt.Errorf("can't be exported.")
...
func (a *AnywhereCacheAdapter) UpdateState(ctx context.Context) error {
	...
	klog.FromContext(ctx).V(2).Info(fmt.Sprintf("Successfully initiated state change for AnywhereCache %s to '%s'", a.id.String(), desiredState))
	return nil
}
